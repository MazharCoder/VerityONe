...
import NewTestCaseModal from '../components/testcase/NewTestCase';

const TestCaseManagement = () => {
  const [showAIGenerator, setShowAIGenerator] = useState<boolean>(false);
  const [showNewTestCase,setShowNewTestCase] = useState<boolean>(false);
    ...
    <div className="h-full animate-[fadeIn_0.3s_ease-in-out]">
      <div className="flex justify-between [@media(max-width:624px)]:flex-col  [@media(max-width:624px)]:gap-2  items-center mb-6">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={()=>setShowNewTestCase(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
            <Plus size={16} />
